...
public class H_State : GroundedState
{
    float ATK_MOVE = 1.2f;
    ...
    public override void handleInput(PlayerController player)
    {
        ...
        else if (stateTime < bufferedInputTrigger)
        {
            ...
            if (Input.GetKeyDown(KeyCode.C)) // H, H
            {
        ...
        else if (stateTime >= bufferedInputTrigger && stateTime <= animationEnd)
        {
            ...
            if (bufferedInput == KeyCode.C) // H, H
            {
                player.state = new HH_State();
                return;
            }
    ...
    public override void update(PlayerController player)
    {
        ...
        if (stateTime >= hitboxStart && !hitboxHasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("H_HitBox"), spawnPoint, player.gameObject.transform.rotation, hitboxDuration);
