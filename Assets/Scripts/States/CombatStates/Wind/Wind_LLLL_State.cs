...
public class Wind_LLLL_State : GroundedState
{
    float ATK_MOVE = 0.9f;
    ...
    public Wind_LLLL_State()
    {
        Debug.Log("Wind_LLLL_State state");
    }
    ...
    public override void handleInput(PlayerController player)
    {
        ...
        else if (stateTime < bufferedInputTrigger)
        {
            ...
            if (Input.GetKeyDown(SettingsInputManager.SIM.lightattack)) // L, L, L, L, L
            {
        ...
        else if (stateTime >= bufferedInputTrigger && stateTime <= animationEnd)
        {
            ...
            if (bufferedInput == SettingsInputManager.SIM.lightattack) // L, L, L, L, L
            {
                player.state = player.currentMask.getLLLLLState();
                return;
            }
    ...
    public override void update(PlayerController player)
    {
        ...
        if (stateTime >= hitboxStart && !hitboxHasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("WindHitBoxes/Wind_LLLL_HitBox"), spawnPoint, player.gameObject.transform.rotation, hitboxDuration);
