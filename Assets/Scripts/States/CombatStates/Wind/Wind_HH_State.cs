...
public class Wind_HH_State : GroundedState
{
    float ATK_MOVE = 0.9f;
    ...
    public Wind_HH_State()
    {
        Debug.Log("Wind_HH_State state");
    }
    ...
    public override void handleInput(PlayerController player)
    {
        ...
        else if (stateTime < bufferedInputTrigger)
        {
            ...
            if (Input.GetKeyDown(SettingsInputManager.SIM.heavyattack)) // H, H, H
            {
        ...
        else if (stateTime >= bufferedInputTrigger && stateTime <= animationEnd)
        {
            ...
            if (bufferedInput == SettingsInputManager.SIM.heavyattack) // H, H, H
            {
                player.state = player.currentMask.getHHHState();
                return;
            }
    ...
    public override void update(PlayerController player)
    {
        ...
        if (stateTime >= hitboxStart && !hitboxHasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("WindHitBoxes/Wind_HH_HitBox"), spawnPoint, player.gameObject.transform.rotation, hitboxDuration);
