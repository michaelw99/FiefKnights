...
public class Wind_LLLH_State : GroundedState
{
    float ATK_MOVE = 0.9f;
    ...
    public Wind_LLLH_State()
    {
        Debug.Log("Wind_LLLH_State state");
    }
    ...
    public override void update(PlayerController player)
    {
        ...
        if (stateTime >= hitboxStart && !hitboxHasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("WindHitBoxes/Wind_LLLH_HitBox"), spawnPoint, player.gameObject.transform.rotation, hitboxDuration);
