...
public class Wind_LLLLL_State : GroundedState
{
    float ATK_MOVE = 0.9f;
    ...
    public Wind_LLLLL_State()
    {
        Debug.Log("Wind_LLLLL_State state");
    }
    ...
    public override void update(PlayerController player)
    {
        ...
        if (stateTime >= hitboxStart && !hitboxHasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("WindHitBoxes/Wind_LLLLL_HitBox"), spawnPoint, player.gameObject.transform.rotation, hitboxDuration);
        ...
        if (stateTime >= hitbox2Start && !hitbox2HasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("WindHitBoxes/Wind_LLLLL_HitBox2"), spawnPoint, player.gameObject.transform.rotation, hitbox2Duration);
            }
            else
            {
                Vector3 spawnPoint = player.gameObject.transform.position + new Vector3(-attackDistance2, 0, 0f);
        ...
        if (stateTime >= hitbox3Start && !hitbox3HasSpawned)
        {
            hitbox3HasSpawned = true;
            if (player.isFacingRight)
            {
                Vector3 spawnPoint = player.gameObject.transform.position + new Vector3(attackDistance2, 0, 0f);
                player.spawnHitbox(Resources.Load("WindHitBoxes/Wind_LLLLL_HitBox3"), spawnPoint, player.gameObject.transform.rotation, hitbox2Duration);
            }
            else
            {
                Vector3 spawnPoint = player.gameObject.transform.position + new Vector3(-attackDistance2, 0, 0f);
