    ...
    public override void update(PlayerController player)
    {
        ...
        if (stateTime >= hitboxStart && !hitboxHasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("LLLH_HitBox"), spawnPoint, player.gameObject.transform.rotation, hitboxDuration);
        ...
        if (stateTime >= hitbox2Start && !hitbox2HasSpawned)
        {
            ...
            if (player.isFacingRight)
            {
                ...
                player.spawnHitbox(Resources.Load("LLLH_HitBox2"), spawnPoint, player.gameObject.transform.rotation, hitbox2Duration);
            }
            else
            {
                Vector3 spawnPoint = player.gameObject.transform.position + new Vector3(-attackDistance2, 0, 0f);
